...
def video_gener(img):
    ...
    det = CarDetector()
    os.chdir(cwd)
    # TEST_IMAGE_PATHS = glob(os.path.join(filepath))
    # TEST_IMAGE_PATHS = glob(os.path.join('test_images/1.jpg'))

    img_full = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    ...
    if len(b):
        ...
        for n in range(len(b)):
            #  [y_up，x_left，y_down，x_right]
            y1 = b[n][0]
            x1 = b[n][1]
            y2 = b[n][2]
            x2 = b[n][3]
            cv2.line(img, (x1, y1), (x1, y2), color=[255, 0, 0], thickness=2)
            cv2.line(img, (x2, y1), (x2, y2), color=[255, 0, 0], thickness=2)
            cv2.line(img, (x1, y1), (x2, y1), color=[255, 0, 0], thickness=2)
            cv2.line(img, (x1, y2), (x2, y2), color=[255, 0, 0], thickness=2)
            cv2.line(img, (100, 400), (900, 400), color=[0, 0, 255], thickness=2)
            if (100 < x1 < 900 ) or (100 < x2 < 900):
                if y2 > 400:
                    img = img_chinese_display(img, '注意车辆', color=(255, 0, 0), position=(50, 100))
    # cv2.namedWindow('input_image', cv2.WINDOW_AUTOSIZE)
    # cv2.imshow('input_image', img)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    # cv2.imwrite('out_img\\' + str(i) + '.jpg', img)
...
# # image test
# img = cv2.imread('cut_imgs/9.jpg')
# out_img = video_gener(img)
#
# write_name = './out_imgs/result'+'9.jpg'
...
clip1 = VideoFileClip('./video_3_cut.mp4')
video_clip = clip1.fl_image(video_gener)
video_clip.write_videofile('video_3_cut_out2.mp4', audio=False)