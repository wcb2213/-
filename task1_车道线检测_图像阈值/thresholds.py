...
def mag_thresh(img, sobel_kernel=3, thresh=(0, 255)):
    ...
    # print(gradmag, '\n', np.max(gradmag), np.min(gradmag) )
    # Scale to 8-bit (0-255) then convert to type = np.uint8
    scaled_gradmag = np.uint8(255*gradmag/np.max(gradmag))
    # print(scaled_gradmag, '\n', np.max(scaled_gradmag), np.min(scaled_gradmag))
    ...
    # print('mag_binary max and min: ', np.max(mag_binary), np.min(mag_binary))
...
def color_thresh(img, r_thresh=(0, 255), s_thresh=(0, 255)):
    ...
    r_channel = img[:,:,2]
    # print('r_channel max and min:', np.max(r_channel), np.min(r_channel))

    r_binary = np.zeros_like(img[:,:,0])
    # Create a mask of 1's where pixel value is within the given thresholds
    r_binary[(r_channel >= r_thresh[0]) & (r_channel <= r_thresh[1])] = 1
    # print('r_binary max and min:', np.max(r_binary), np.min(r_binary))

    # Convert to HLS color space
    hls = cv2.cvtColor(img, cv2.COLOR_BGR2HLS)
    # Apply a threshold to the S channel
    s_channel = hls[:,:,2]
    # print('s_channel max and min:', np.max(s_channel), np.min(s_channel))

    s_binary = np.zeros_like(s_channel)
    # Create a mask of 1's where pixel value is within the given thresholds
    s_binary[(s_channel >= s_thresh[0]) & (s_channel <= s_thresh[1])] = 1
    # print('s_binary max and min:', np.max(s_binary), np.min(s_binary))

    # Combine two channels
    combined = np.zeros_like(img[:,:,0])
    combined[(s_binary == 1) | (r_binary == 1)] = 1
    # print('combined max and min:', np.max(combined), np.min(combined))
    # Return binary output image
    return combined
